...
public final class HashTable<K, V> {
	...
	public void clear() {
		for (int i = 0; i < array.length; i++) {
			ArrayList<Entry<K, V>> list = (ArrayList<Entry<K, V>>) array[i];
			list.clear();
		}
	}
	...
	public boolean contains(V value) {
		...
		for (int i = 0; i < array.length; i++) {
			ArrayList<Entry<K, V>> list = (ArrayList<HashTable<K, V>.Entry<K, V>>) array[i];
	...
	public boolean containsKey(K key) {
		...
		ArrayList<Entry<K, V>> list = (ArrayList<HashTable<K, V>.Entry<K, V>>) array[indexKey];
	...
	public V get(K key) {
		...
		int indexKey = indexOf(key);
		ArrayList<Entry<K, V>> list = (ArrayList<Entry<K, V>>) array[indexKey];
		for (int i = 0; i < list.size(); i++) {
			if (list.get(i).key.equals(key)) {
	...
	public V replace(K key, V value) {
		...
		ArrayList<Entry<K, V>> list = (ArrayList<HashTable<K, V>.Entry<K, V>>) array[indexKey];
		for (int i = 0; i < list.size(); i++) {
			Entry<K, V> listEntry = list.get(i);
			if (listEntry.key.equals(key)) {
				V previousValue = listEntry.value;
	...
	public V put(K key, V value) {
		...
		ArrayList<Entry<K, V>> list = (ArrayList<HashTable<K, V>.Entry<K, V>>) array[indexOf(newEntry.key)];
		if (containsKey(key)) {
	...
	protected void rehash() {
		...
		for (int i = 0; i < oldArray.length; i++) {
			ArrayList<Entry<K, V>> entryList = (ArrayList<HashTable<K, V>.Entry<K, V>>) oldArray[i];
			for (int j = 0; j < entryList.size(); j++) {
				put(entryList.get(j).key, entryList.get(j).value);
			}
		}
	}
